...
const { getAllUsers, createUser, updateUser, getUserByUsername, deleteUsers } = require('../models/usersModel');
...
const deleteUsersController = async (req, res) => {
    const { userIds } = req.body; // 삭제할 유저들의 ID 배열

    if (!userIds || userIds.length === 0) {
        return res.status(400).json({ error: '삭제할 유저 ID가 필요합니다.' });
    }
    ...
    try {
        ...
        res.status(200).json({
            message: `${deletedUsers.length}명의 유저가 성공적으로 삭제되었습니다.`,
            deletedUsers,
        });
    } catch (error) {
        res.status(500).json({
